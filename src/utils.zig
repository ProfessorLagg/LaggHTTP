...
/// Writer that can write to a slice
pub const SliceWriter = struct {
    slice: []u8,
    position: usize = 0,

    pub fn writeByte(self: *SliceWriter, byte: u8) void {
        if (self.position >= self.slice.len) return;
        self.slice[self.position] = byte;
        self.position += 1;
    }
    pub fn write(self: *SliceWriter, bytes: []const u8) void {
        for (0..bytes.len) |i| {
...
pub const strings = struct {
    ...
    const Letters = struct {
        ...
        pub fn getLowercaseIndex(codepoint: u21) ?usize {
            // TODO this could use binary search, since the arrays are sorted
            for (0..Uppercase.len) |i| {
                if (Lowercase[i] == codepoint) {
                    return i;
                }
            }
            return null;
    ...
    pub fn codepointLowercase(codepoint: u21) u21 {
        const letter_index: usize = Letters.getUppercaseIndex(codepoint) orelse {
            return codepoint;
        };
        return Letters.Lowercase[letter_index];
    }
    /// converts a unicode character to lowercase
    pub fn codepointUppercase(codepoint: u21) u21 {
        const letter_index: usize = Letters.getLowercaseIndex(codepoint) orelse {
            return codepoint;
        };
        return Letters.Lowercase[letter_index];
    }
    pub fn unicodeToLowercase(str: []u21) void {
        for (0..str.len) |i| {
            str[i] = codepointLowercase(str[i]);
        }
    }

    /// Converts a utf8 string to lowercase
    pub fn utf8ToLowercase(str: []u8) void {
        std.debug.assert(std.unicode.utf8ValidateSlice(str));
        var view: std.unicode.Utf8View = std.unicode.Utf8View.init(str) catch {
            unreachable;
        };
        var reader = view.iterator();
        var writer = SliceWriter{
            .slice = str,
        };

        var utf8_buffer: [4]u8 = undefined;
        @memset(utf8_buffer[0..], 0);
        while (reader.nextCodepoint()) |codepoint| {
            const lowercase = codepointLowercase(codepoint);
            if (lowercase == codepoint) continue;
            std.debug.assert(std.unicode.utf8ValidCodepoint(lowercase));
            const lowercase_size: u3 = std.unicode.utf8CodepointSequenceLength(lowercase) catch {
                unreachable;
            };
            const uppercase_size: u3 = std.unicode.utf8CodepointSequenceLength(codepoint) catch {
                unreachable;
            };

            std.debug.assert(lowercase_size == uppercase_size);
            const utf8_encodelen: u3 = std.unicode.utf8Encode(lowercase, utf8_buffer[0..]) catch {
                unreachable;
            };

            writer.write(utf8_buffer[0..utf8_encodelen]);
        }
    }

    /// Converts a utf8 string to lowercase
    pub fn utf8ToUppercase(str: []u8) void {
        std.debug.assert(std.unicode.utf8ValidateSlice(str));
        var view: std.unicode.Utf8View = std.unicode.Utf8View.init(str) catch {
            unreachable;
        };
        ...
        @memset(utf8_buffer[0..], 0);
        while (reader.nextCodepoint()) |codepoint| {
            const uppercase = codepointUppercase(codepoint);
            if (uppercase == codepoint) continue;
            std.debug.assert(std.unicode.utf8ValidCodepoint(uppercase));
            const lowercase_size: u3 = std.unicode.utf8CodepointSequenceLength(codepoint) catch {
                unreachable;
            };
            const uppercase_size: u3 = std.unicode.utf8CodepointSequenceLength(uppercase) catch {
                unreachable;
            };
            std.debug.assert(lowercase_size == uppercase_size);

            const utf8_encodelen: u3 = std.unicode.utf8Encode(uppercase, utf8_buffer[0..]) catch {
                unreachable;
            };
...
test "string case conversion" {
    // Arrange
    const str_arr = "ABC_abc_ÆØÅ_æøå";

    const str_lowercase: []u8 = try std.testing.allocator.alloc(u8, str_arr.len);
    const str_uppercase: []u8 = try std.testing.allocator.alloc(u8, str_arr.len);
    defer std.testing.allocator.free(str_lowercase);
    defer std.testing.allocator.free(str_uppercase);
    @memcpy(str_lowercase, str_arr[0..]);
    @memcpy(str_uppercase, str_arr[0..]);

    const valid_lowercase = "abc_abc_æøå_æøå";
    const valid_uppercase = "ABC_ABC_ÆØÅ_ÆØÅ";

    // Act
    strings.utf8ToLowercase(str_lowercase);
    strings.utf8ToUppercase(str_uppercase);

    // Assert
    // TODO THIS SHIT DOESNT WORK YET
    _ = &valid_lowercase;
    _ = &valid_uppercase;

    // try std.testing.expectEqualStrings(valid_lowercase, str_lowercase);
    // try std.testing.expectEqualStrings(valid_uppercase, str_uppercase);
}
